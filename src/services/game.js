...
class GameClass {
  game_config_info = null
  enemy_in_team_info = null
  enemy_character_type_info = null
  enemy_standard_attribute_info = null

  constructor() {
    this.game_config_info = window.mDB.game_config_info
    this.enemy_in_team_info = window.mDB.enemy_in_team_info
    this.enemy_character_type_info = window.mDB.enemy_character_type_info
    this.enemy_standard_attribute_info = window.mDB.enemy_standard_attribute_info
  }
  ...
  getEnemyCharAtLevel(enemy_character_type_id, level, number) {
    var enemyChar = this.enemy_character_type_info[enemy_character_type_id];
    var lv_from = this.enemy_standard_attribute_info[enemyChar.level];
    var lv_to = this.enemy_standard_attribute_info[level];
  ...
  getEnemyPower(enemy, def_percent) {
    var eea = this.breakStringArray(this.game_config_info.enemy_effect_attack.parameter_value, s => f32(s));
    var eed = this.breakStringArray(this.game_config_info.enemy_effect_defence.parameter_value, s => f32(s));
  ...
  getEnemyTeamPower(enemyTeam, turnNo) {
    ...
    $.each(enemyTeam.member_ids, (index, member_ids) => {
      var member = this.enemy_in_team_info[member_ids];
